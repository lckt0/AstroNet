...
namespace Runtime.Execution.Internal {
	internal static class VMTrampoline {
		...
		static VMTrampoline() {
			foreach (var method in typeof(AstroNet).GetMethods(BindingFlags.Static | BindingFlags.NonPublic)) {
		...
		static DynamicMethod CreateTrampolineNormal(int moduleId, ulong codeAdr, uint key, VMFuncSig sig, uint sigId) {
			...
			//body.Instructions.Add(Instruction.Create(OpCodes.Ldstr, "AstroNet - github.com/lckt0"));
			...
			gen.Emit(System.Reflection.Emit.OpCodes.Ldstr, "AstroNet - github.com/lckt0");
			gen.Emit(System.Reflection.Emit.OpCodes.Ldc_I4, (int)moduleId);
